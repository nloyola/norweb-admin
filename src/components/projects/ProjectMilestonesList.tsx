...
export function ProjectMilestonesList() {
  ...
  const {
    ...
    ['projects', projectId, 'milestones', page, searchTerm],
    () => ProjectsApi.paginateMilestones(projectId, page, searchTerm || ''),
    {
  ...
  const handlePageChange = (_milestone: ChangeEvent<unknown>, newPage: number) => {
    if (page !== newPage) {
      setPage(newPage);
    }
  };
  ...
  return (
    <>
      ...
      {!isLoading && pagination && (
        <>
          {/*
          <ProjectMilestonesTable projectMilestones={pagination.results || []} />
            */}
          {pagination.pages > 1 && (
            <Box
              sx={{
